...
import { LibraryCatalogDto } from '../../share/models/library-catalog-dto.model';
...
export class IndexComponent implements OnInit {
  ...
  isLoading = true;
  total = 0;
  data: LibraryCatalogDto[] = [];
  columns: string[] = ['name',　'type',　'sort',　'actions'];
  dataSource!: MatTableDataSource<LibraryCatalogDto>;
    ...
    this.service.filter(this.filter)
      .subscribe(res => {
        if (res.data) {
          ...
          this.dataSource = new MatTableDataSource<LibraryCatalogDto>(this.data);
        }
      });
  }

  deleteConfirm(item: LibraryCatalogDto): void {
  ...
  delete(item: LibraryCatalogDto): void {
...
openEditDialog(id: string): void {
  ...
  edit(id: string): void {
    console.log(id);
    this.router.navigateByUrl('/library-catalog/edit/' + id);
  }
